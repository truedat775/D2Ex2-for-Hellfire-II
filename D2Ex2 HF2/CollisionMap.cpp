...
BOOL CCollisionMap::DumpMap(LPCSTR lpszFilePath,  const vector<COORDS> cPath) const
{
	...
	POINT ptPlayer = {pMe->pPath->xPos,pMe->pPath->yPos};
...
//								LeaveCriticalSection(&CriticalSection);
		...
		if(!pRoom->pRoom1)
		{
			D2Funcs.D2COMMON_AddRoomData(pMe->pAct, pRoom->pLevel->dwLevelNo, pRoom->dwPosX, pRoom->dwPosY, pMe->pPath->pRoom1);
			bAdded = TRUE;
		}
			...
			for(PresetUnit* pUnit = pRoom->pPreset; pUnit; pUnit = pUnit->pPresetNext)
			{
				...
				{
					if(bAdded)
						D2Funcs.D2COMMON_RemoveRoomData(pMe->pAct, pRoom->pLevel->dwLevelNo, pRoom->dwPosX, pRoom->dwPosY, pMe->pPath->pRoom1);
//					LeaveCriticalSection(&CriticalSection);
					return FALSE;
				}
