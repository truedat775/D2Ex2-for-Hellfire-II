...
// Path Finding Result
/////////////////////////////////////////////////////////////////////
enum {   INT_PATH_FAIL = 0,     // Failed, error occurred or no available path
		 INT_PATH_CONTINUE,	    // Path OK, destination not reached yet
		 INT_PATH_REACHED };    // Path OK, destination reached(Path finding completed successfully)
...
{	
	...
	{
		pos = m_ptEnd;
		return INT_PATH_REACHED; // we reached the destination
	}

	if (!IsValidIndex(pos.x, pos.y))
		return INT_PATH_FAIL; // fail
	...
	if (value >= RANGE_INVALID)
		return INT_PATH_FAIL; // no path at all	
	
	pos = best;
	Block(pos, nAdjust);	
	return INT_PATH_CONTINUE; // ok but not reached yet
}
...
vector<COORDS> CTeleportPath::FindTeleportPath(COORDS ptStart, COORDS ptEnd)
{
	...
	while(nRes != INT_PATH_FAIL)
	{

		if (nRes != INT_PATH_REACHED && ExAim::CalculateDistance(v_Output.back(), pos) < 20)
		{
