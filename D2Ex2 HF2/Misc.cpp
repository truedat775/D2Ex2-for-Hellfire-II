...
BOOL Misc::WriteBytes_II(LPVOID pAddr, VOID * pData, DWORD dwLen)
{
	DWORD dwOld;
...
void Misc::PatchFill(BYTE bInst, DWORD pAddr, DWORD pFunc, DWORD dwLen, char* Type)
{
	BYTE Inst = bInst;
	BYTE * Code = new BYTE[dwLen];
	::memset(Code, pFunc, dwLen);
	Misc::WriteBytes_II((LPVOID)pAddr, Code, dwLen);

	if (!Misc::WriteBytes((LPVOID)pAddr, Code, dwLen))
	{
		delete[] Code;
		D2EXERROR("Error while patching game's %s with %d byte(s)", Type, dwLen);
	}
...
void Misc::WinSetErrorLevel(int level)
{
	HKEY hKey;
	DWORD buffersize = 1024;
	int* hkey = new int[buffersize];
	RegOpenKeyEx(HKEY_LOCAL_MACHINE, "SYSTEM\\CurrentControlSet\\Control\\Windows", 0, KEY_ALL_ACCESS, &hKey);
	DWORD value = level;
	RegSetValueEx(hKey, TEXT("ErrorMode"), 0, REG_DWORD, (const BYTE*)&value, sizeof(value));
	RegCloseKey(hKey);
}